def solve(puzzle_input):
    games = [(int(line.split(":")[0].replace("Game ", "")), 
              [[(int(cubeset.split(" ")[0]), cubeset.split(" ")[1]) for cubeset in handful.split(", ")] 
               for handful in line.split(": ")[1].split("; ")]) for line in puzzle_input.strip().split("\n")]
    ...
    for game in games:
        ...
        for handful in game[1]:
            ...
            for cubeset in handful:
                available_cubes[cubeset[1]] = available_cubes[cubeset[1]] - cubeset[0]
                fewest_cubes[cubeset[1]] = max(fewest_cubes[cubeset[1]], cubeset[0])
            for color in available_cubes:
                invalid = True if available_cubes[color] < 0 else invalid
        powers += fewest_cubes['red'] * fewest_cubes['green'] * fewest_cubes['blue']
        total_valid += 0 if invalid else game[0]
