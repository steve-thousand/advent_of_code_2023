...
def solve(puzzle_input):
    vertices = {line.split(": ")[0]:line.split(": ")[1].split(" ") for line in puzzle_input.strip().splitlines()}
    ...
    if sys.argv[0] == 'graph':
        ...
        edges_to_remove = [('cvx', 'tvj'), ('spx', 'fsv'), ('kdk', 'nct')]
        for removed in edges_to_remove:
            if removed[0] in vertices:
                if removed[1] in vertices[removed[0]]:
                    vertices[removed[0]].remove(removed[1])
        ...
        while found:
            ...
            for vertex in vertices.keys():
                for adj in vertices[vertex]:
                    if adj not in vertices:
                        not_matched.add(adj)
                        found = True
                    else:
                        vertices[adj].append(vertex)
            for m in not_matched:
                vertices[m] = []

        start = list(vertices.keys())[0]
        visited = set([start])
        queue = [start]
        while len(queue) > 0:
            vertex = queue.pop()
            for adj in vertices[vertex]:
                if adj not in visited:
                    visited.add(adj)
                    queue.append(adj)
        answer = len(visited) * (len(vertices.keys()) - len(visited))
        print(answer)
