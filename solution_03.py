...
def get_adjacent_numbers(gear_y, gear_x, schematic):
    ...
    for y in range(gear_y - 1, gear_y + 2):
        if y >= 0 and y < len(schematic):
            ...
            for x in range(0, len(schematic[y])):
                this = schematic[y][x]
                is_digit = ord(this) >= 48 and ord(this) <= 57
                if is_digit:
                    number_digits.append(this)
                    for dy in range(0, 3):
                        for dx in range(0, 3):
                            if y + dy - 1 == gear_y and x + dx - 1 == gear_x:
                                is_adjacent = True
                if not is_digit or x == len(schematic[y]) - 1:
                    if is_adjacent:
                        number = None if len(number_digits) == 0 else int(''.join(number_digits))
                        if number is not None and is_adjacent:
                            gear_adjacent_numbers.append(number)
                    is_adjacent = False
                    number_digits = []
    return gear_adjacent_numbers

def solve(puzzle_input):
    schematic = [[x for x in y] for y in puzzle_input.strip().split("\n")]
            ...
            if is_digit:
                ...
                for dy in range(0, 3):
                    for dx in range(0, 3):
                        check_y = y + dy - 1
                        check_x = x + dx - 1
                        if check_y >= 0 and check_y < len(schematic) and check_x >= 0 and check_x < width:
                            adjacent = schematic[check_y][check_x]
                            if adjacent != "." and (ord(adjacent) < 48 or ord(adjacent) > 57):
                                is_adjacent = True
            if is_gear:
                gear_adjacent_numbers = get_adjacent_numbers(y, x, schematic)
                if len(gear_adjacent_numbers) == 2:
                    gear_ratios.append(gear_adjacent_numbers[0] * gear_adjacent_numbers[1])
            if (not is_digit or x == width - 1) and in_number:
