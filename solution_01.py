...
def find_lowest_i(string, digit_string):
    try:
        return string.index(digit_string)
    except ValueError as e: 
        # can't believe this throws
        return -1
    
def find_highest_i(string, digit_string):
    try:
        return string.rindex(digit_string)
    except ValueError as e: 
        return -1
    
def calibration_value(line, include_letters = False):
    lowest_i = 999999
    lowest_digit = 0
    highest_i = -1
    highest_digit = 0
    for digit in range(0,10):
        for i in range (0, 2 if include_letters else 1):
            if digit == 0:
                continue
            if i == 1:
                digit_string = DIGIT_NAMES[digit - 1]
            else:
                digit_string = str(digit)

            lindex = find_lowest_i(line, digit_string)
            if lindex != -1 and lindex < lowest_i:
                lowest_i = lindex
                lowest_digit = digit

            rindex = find_highest_i(line, digit_string)
            if rindex != -1 and rindex > highest_i:
                highest_i = rindex
                highest_digit = digit
    return int(str(lowest_digit) + '' + str(highest_digit))

def solve(puzzle_input):
    lines = [x for x in puzzle_input.strip().split("\n")]

    solution_1 = 0
    for line in lines:
        solution_1 += calibration_value(line)
    print(solution_1)

    solution_2 = 0
    for line in lines:
        solution_2 += calibration_value(line, True)
    print(solution_2)
