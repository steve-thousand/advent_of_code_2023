...
def parse_rule(rule_string):
    comparator = "<" if "<" in rule_string else ">" if ">" in rule_string else None
    if comparator is None:
        return (lambda x: True, rule_string)
    check = rule_string.split(":")[0].split(comparator)[0]
    compare_value = int(rule_string.split(":")[0].split(comparator)[1])
    next_workflow = rule_string.split(":")[1]
    compare = (lambda x: x[check] > compare_value) if comparator == ">" else (lambda x: x[check] < compare_value)
    return (compare, next_workflow, check, comparator, compare_value)

def check_possible(workflow, parts_min, parts_max, workflows):
    found = 0
    for rule_string in workflow:
        parsed = parse_rule(rule_string)
        left_parts_min = copy.deepcopy(parts_min)
        left_parts_max = copy.deepcopy(parts_max)
        if len(parsed) > 2:
            if parsed[3] == '<':
                left_parts_max[parsed[2]] = min(left_parts_max[parsed[2]], parsed[4] - 1)
                parts_min[parsed[2]] = max(parts_min[parsed[2]], parsed[4])
            if parsed[3] == '>':
                left_parts_min[parsed[2]] = max(left_parts_min[parsed[2]], parsed[4] + 1)
                parts_max[parsed[2]] = min(parts_max[parsed[2]], parsed[4])
        if parsed[1] == 'A':
            found += functools.reduce(lambda x, y: x*y if x*y > 0 else 0, [x[1] - x[0] + 1 for x in list(zip(left_parts_min.values(), left_parts_max.values()))])
        elif parsed[1] != 'R':
            found += check_possible(workflows[parsed[1]], copy.deepcopy(left_parts_min), copy.deepcopy(left_parts_max), workflows)
    return found

def solve(puzzle_input):
    workflows = {y.split("{")[0]:y.split("{")[1][:-1].split(",") for y in puzzle_input.strip().split("\n\n")[0].splitlines()}
    parts = [{x.split("=")[0]:int(x.split("=")[1]) for x in y[1:-1].split(",")} for y in puzzle_input.strip().split("\n\n")[1].splitlines()]
    ...
    print(sum([sum(list(x.values())) for x in accepted]))

    found = check_possible(workflows['in'], {'x': 1, 'm': 1, 'a': 1, 's': 1}, {'x': 4000, 'm': 4000, 'a': 4000, 's': 4000}, workflows)
    print(found)
