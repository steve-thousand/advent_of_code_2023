...
def clear_nonloop(grid, closest):
    for y in range(0, len(grid)):
        for x in range(0, len(grid[y])):
            char_at = grid[y][x]
            if char_at != '.' and (y, x) not in closest:
                grid[y][x] = '.'
...
def define_start(grid):
    ...
    down = False
    right = False
    if y + 1 < len(grid) and grid[y + 1][x] == 'J' or grid[y + 1][x] == 'L' or grid[y + 1][x] == '|':
        down = True
    if x + 1 < len(grid[y]) and grid[y][x + 1] == 'J' or grid[y][x + 1] == '7' or grid[y][x + 1] == '-':
        right = True
    if down:
        return 'F' if right else '7'
    else:
        return 'L' if right else 'J'

def solve(puzzle_input):
    grid = [[x for x in y] for y in puzzle_input.strip().split("\n")]

    start_node = find_start(grid)
    start_char = define_start(grid)
    grid[start_node[0]][start_node[1]] = start_char
    ...
    while len(queue) > 0:
        ...
        if left and x - 1 >= 0 and ((y, x - 1) not in closest or closest[(y, x - 1)] > distance + 1):
            queue.append(((y, x - 1), distance + 1))
        if right and x + 1 < len(grid[y]) and ((y, x + 1) not in closest or closest[(y, x + 1)] > distance + 1):
            queue.append(((y, x + 1), distance + 1))
        if up and y - 1 >= 0 and ((y - 1, x) not in closest or closest[(y - 1, x)] > distance + 1):
            queue.append(((y - 1, x), distance + 1))
        if down and y + 1 < len(grid) and ((y + 1, x) not in closest or closest[(y + 1, x)] > distance + 1):
            queue.append(((y + 1, x), distance + 1))

    print(max(closest.values()))

    total_interior = 0
    clear_nonloop(grid, closest)
    for y in range(0, len(grid)):
        for x in range(0, len(grid[y])):
            if grid[y][x] == '.':
                dx = 1
                ...
                on_edge = None
                crosses = 0
                while check_x < len(grid[y]):
                    char_at = grid[y][check_x]
                    if char_at == '|':
                        crosses += 1
                    elif char_at == 'F':
                        on_edge = 'J'
                        ...
                        on_edge = '7'
                    elif char_at == '7' or char_at == 'J':
                        crosses += 1 if char_at == on_edge else 0
                    check_x += dx
                if crosses % 2 > 0:
                    total_interior += 1
